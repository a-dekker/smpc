...
ArtistModel::ArtistModel(QList<MpdArtist *> *list, ImageDatabase *DB,
                         QObject *parent)
...
QVariant ArtistModel::data(const QModelIndex &index, int role) const {
    ...
    if (role == NameRole)
        ...
        if (imageID == 0 || imageID == -1) {
            ...
            qDebug() << "returning dummy image for artist: "
                     << tmpArtist->getName();
            // emit requestAlbumInformation(*album);
            //  Return dummy for the time being
            return DUMMY_ARTISTIMAGE;
        } else if (imageID == -2) {
            qDebug() << "returning dummy image for blacklisted artist: "
                     << tmpArtist->getName();
            return DUMMY_ARTISTIMAGE;
        } else {
            qDebug() << "returning database image for album: "
                     << tmpArtist->getName();
            QString url =
                "image://imagedbprovider/artistid/" + QString::number(imageID);
            return url;
        }
    }
    return QVariant(QVariant::Invalid);
