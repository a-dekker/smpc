...
Component {
	ListItem {
		...
		Image {
			...
			source: isDirectory
				...
				: isPlaylist
					...
					: imageURL
			height: filenametext.height + trackLabel >= Theme.itemSizeSmall
				? filenametext.height + trackLabel
				: Theme.itemSizeSmall
			width: height
		}
		...
		onClicked: {
			...
			if (isFile) {
				albumTrackClicked(title, album, artist, length, path, year, tracknr, trackmbid, artistmbid, albummbid)
			}
			if (isPlaylist) {
