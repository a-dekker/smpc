					...
					Column {
						...
						Column {
							...
							ToggleImage {
								...
								id: coverImage
								property int calcheight: (infoFlickable.height - (titleText.height + albumText.height + artistText.height))
								height: showCoverNowPlaying
									? calcheight > contentColumn.width - listPadding * 2
										? contentColumn.width - listPadding * 2
										: calcheight
									: 0
								width: height
	...
	onStatusChanged: {
		if (status === PageStatus.Activating) {
			...
			if (mCurrentSongPage == undefined) {
				var currentSongComponent = Qt.createComponent(Qt.resolvedUrl("CurrentSong_large.qml"))
